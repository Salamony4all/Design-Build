...
export async function generateCompletePPTX() {
    try {
        const project = useBOQStore.getState().project || { name: 'New Project', client: 'Design & Build' };
        ...
        const applyBranding = (slide) => {
            ...
            // Footer bar
            slide.addShape(pptx.ShapeType.rect, { x: 0, y: 6.8, w: '100%', h: 0.7, fill: BRAND_COLORS.darker });
        ...
        console.log('[Export] Generating Slide 1: Title...');
        ...
        console.log('[Export] Generating Slide 2: Philosophy...');
        ...
        console.log('[Export] Generating Slide 3: Space Analysis...');
        ...
        slide3.addText(`${detectedRooms.length} Zones Identified | ${(surveyor.floorArea || 0).toFixed(1)} m² Total`, {
        ...
        console.log('[Export] Generating Slide 4: Materials...');
        ...
        console.log('[Export] Generating Slide 5: Floor Plan...');
        ...
        console.log('[Export] Generating Slide 6: 3D Visualization...');
        // ============================================================================
        ...
        keyRooms.forEach((room, idx) => {
            console.log(`[Export] Generating Slide ${7 + idx}: Room Detail ${room.label}...`);
        ...
        // Save file
        console.log('[Export] Finalizing PPTX file write...');
        const fileName = `${project.name?.replace(/[\s\/\*\?\:\"<>\|]+/g, '_') || 'Design_Proposal'}_${new Date().toISOString().split('T')[0]}.pptx`;
        await pptx.writeFile({ fileName });
        console.log('[Export] PPTX Generation SUCCESS:', fileName);

        return { success: true, fileName };
    } catch (error) {
        console.error('[Export Service] FAILED to generate Branded Presentation:', error);
        throw error;
    }
}
